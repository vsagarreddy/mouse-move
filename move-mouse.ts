...
import { mouse, screen, Point, straightTo } from "@nut-tree-fork/nut-js";

mouse.config.mouseSpeed = 500;

async function moveMouse() {
    const margin = 20; 
  const screenSize = await screen.width(); // ✅ Correct way to get screen width
  const screenHeight = await screen.height(); // ✅ Correct way to get screen height

  const randomX = Math.floor(Math.random() * screenSize - margin);
  const randomY = Math.floor(Math.random() * screenHeight - margin);

  await mouse.move(straightTo(new Point(randomX, randomY)));
}
